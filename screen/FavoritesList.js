...
const FavoritesList = (props) => {
  ...
  useEffect(() => {
    ...
    fetch("http://192.168.16.108:3000/tb_products")
  ...
  const themGioHang = (item) => {
    ...
    axios
      .post("http://192.168.16.108:3000/tb_giohang", data)
      .then((response) => {
  ...
  const renderProductItem = ({ item }) => (
    <View style={styles.itemSanPham}>
      <TouchableOpacity>
        <Image
          style={{ width: 130, height: 120, marginLeft: 10 }}
  ...
  // Lọc dữ liệu sản phẩm dựa trên giá trị tìm kiếm
    ...
    <KeyboardAvoidingView style={styles.container}>
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          {/* List */}
          <FlatList
            data={filteredProducts} // Sử dụng dữ liệu đã lọc
